const colors = { reset: '\x1b[0m', okay: '\x1b[32m', warn: '\x1b[33m', error: '\x1b[31m' };

function log(message) {
  console.log(`${colors.reset}${new Date().toLocaleTimeString()} ${message}${colors.reset}`);
}
...
function giveServerResponse(httpStatus, data, contentType = 'application/json') {
  const color = httpStatus < 300 
    ? colors.okay 
    : (httpStatus < 500 ? colors.warn : colors.error);
  ...
  httpStatus === 204
    ? this.end()
    : this.end(contentType === 'application/json' ? JSON.stringify(data) : data);
}
...
module.exports = { log, getPostBody, giveServerResponse, isForIncomingMessage, compareBookingsDescending };
