...
const { log, getPostBody, giveServerResponse, isForIncomingMessage, compareBookingsDescending } = require ('./util');

const PORT = process.env.PORT || 5000;
const SIMULATED_INSTABILITY = parseFloat(process.env.SIMULATED_INSTABILITY || 0.05);
...
const server = http.createServer(async (request, response) => {
  ...
  // GET /
    ...
    response.give(200, 'Your server is running!\n\n----\n\n' + readme, 'text/markdown');
  }

  // Simulate somewhat unstable server:
  else if (Math.random() < SIMULATED_INSTABILITY) {
    Math.random() < 0.75
      ? response.give(500, 'An internal server error occurred (simulated by server)')
  ...
  // GET /api/v1/bookings
    ...
    response.give(200, bookings.sort(compareBookingsDescending).slice(0, limit));
  }
  ...
  // GET /api/v1/bookings/{id}
    ...
    booking ? response.give(200, booking) : response.give(404, { message: 'Not Found' });
  }
  ...
  // GET /api/v1/bookings/front-desk-alert
    ...
    response.give(200, { frontDeskAlerts, fetchedAtUtc: new Date().toISOString() });
  }

  // POST /api/v1/bookings/front-desk-alert
  else if (request.isFor('POST', /^\/api\/v1\/bookings\/front-desk-alert\/?$/)) {
    try {
      ...
      frontDeskAlerts.push({
        ...
        reason: body.reason || 'No reason provided, but please review this booking.',
...
server.listen(PORT, () => log(`Server ready at port ${PORT} - simulating server errors with probability of ${SIMULATED_INSTABILITY}`));
